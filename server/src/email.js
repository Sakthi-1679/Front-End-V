// email.js – Brevo (Sendinblue) helper
// Sends admin-notification e-mails for new orders.
...
// Build and cache a Brevo API client once.
let _client = null;
function getClient() {
  if (!_client) {
...
/**
 * Verify that the Brevo API key is reachable by fetching the account info.
...
async function checkBrevoConnection() {
  ...
  try {
    ...
    return { ok: true, account: account.body };
  } catch (err) {
...
/**
 * Send a new-order alert to the admin.
...
async function sendAdminAlert({ subject, htmlContent }) {
  ...
  try {
    const result = await getClient().sendTransacEmail(sendSmtpEmail);
...
/**
 * Build and send the admin notification for a standard product order.
 */
...
/**
 * Build and send the admin notification for a custom order request.
 */
...
module.exports = { checkBrevoConnection, notifyAdminNewOrder, notifyAdminNewCustomOrder };
